# Author: kmmcelro
# This is my solution to the Hackerrank challenge, Class 2 - Find the Torsional Angle
...
class Points(object):
    ...
    def dot(self, no):
        dotp = self.x*no.x + self.y*no.y + self.z*no.z
        return dotp
        
    def cross(self, no):
        crossp = Points(0,0,0)
        crossp.x = self.y * no.z - self.z * no.y
        crossp.y = self.z * no.x - self.x * no.z
        crossp.z = self.x * no.y - self.y * no.x
        return crossp
